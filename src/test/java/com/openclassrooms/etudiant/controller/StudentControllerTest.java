...
class StudentControllerTest {
                ...
                void shouldReturn404_WhenEmailNotFound() throws Exception {
                        ...
                        when(studentService.getStudentByEmail("unknown@example.com"))
                                        .thenThrow(new EntityNotFoundException(
                                                        "Student not found with email: unknown@example.com"));
                        ...
                        mockMvc.perform(get("/api/students/email/unknown@example.com")
                                        ...
                                        .andExpect(jsonPath("$.message")
                                                        .value("Student not found with email: unknown@example.com"));
                }
        ...
        class UpdateStudentEndpoint {
                ...
                // : branche DataIntegrityViolationException dans PUT
                // non couverte
                @Test
                @DisplayName("Should return 409 when service throws DataIntegrityViolationException")
                void shouldReturn409_WhenDataIntegrityViolated() throws Exception {
                        /* ARRANGE - DB constraint violation on update */
                        Student studentNoId = Student.builder().firstName("Alice").lastName("Smith")
                                        .email("alice@example.com").address("1 rue de la Paix")
                                        .city("Paris").zipCode("75001").build();
                        when(studentMapper.toEntity(any(StudentDTO.class))).thenReturn(studentNoId);
        ...
        class DeleteStudentEndpoint {
                ...
                // : branche IllegalArgumentException dans DELETE non
                // couverte
                @Test
                @DisplayName("Should return 400 when service throws IllegalArgumentException on delete")
                void shouldReturn400_WhenIllegalArgumentThrown() throws Exception {
                        /* ARRANGE - service rejects null/invalid ID */
                        doThrow(new IllegalArgumentException("Student ID cannot be null"))
